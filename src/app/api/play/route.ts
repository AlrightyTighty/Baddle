...
export function SOCKET(connection: WebSocket, request: IncomingMessage) {
  ...
  let { name, roomCode, makeRoom, selectedIcon } = parsedUrl.query as { [key: string]: string };
  ...
  if (makeRoom == "true") {
    ...
    const newPlayer = new Player(name, connection, newGame, true, parseInt(selectedIcon));
    ...
    if (Game.activeGames[roomCode]) {
      ...
      if ((!game.started || game.options.allowLateJoin) && game.players.length < game.options.roomSize) {
        const newPlayer = new Player(name, connection, game, false, parseInt(selectedIcon));
